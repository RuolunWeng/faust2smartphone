...
- (void)viewDidLoad {
    ...
    dspFaust->setParamValue("/Oscillator/volume", 0);
    dspFaust->setParamValue("/Oscillator/freq", 1);
...
- (void)startMotion
{
    ...
    _motionTimer = [NSTimer scheduledTimerWithTimeInterval:1.f/(SR/bufferSize) target:self
...
- (void)updateGUI
{
    ...
    float param3 = (dspFaust->getOutput(0) + 1.0)/2.0;
    self.view.alpha = param3;
}
...
- (IBAction)freqOut:(id)sender {
    ...
    _freqValue.text=[NSString stringWithFormat: @"%.3fHz", self.freq.value];
}
@end
