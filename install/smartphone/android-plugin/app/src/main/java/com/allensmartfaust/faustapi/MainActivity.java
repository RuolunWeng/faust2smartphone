...
public class MainActivity extends AppCompatActivity {
    ...
    if ((currentTime-lastDate) > updateInterval) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            dspFaust.propagateAcc(0, event.values[0]*(-9.8f));
            dspFaust.propagateAcc(1, event.values[1]*(-9.8f));
            dspFaust.propagateAcc(2, event.values[2]*(9.8f));
            }
        ...
        getParam1 = (dspFaust.getOutput(0)+1.f) / 2.f;
    ...
    protected void onStart() {
        ...
        if (!isChangingConfigurations()) {
            if(checkPermission()){
                ...
                dspFaust.setParamValue("/Oscillator/freq",0.1f);
                paramOut1.setText("Freq:0.1Hz");
                param1.setProgress(1);
